    ...
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
      ...
      <form @submit.prevent="submitForm" class="space-y-4">
        ...
        <div>
          ...
          <p class="mt-1 text-xs text-gray-500">Range: 1:1 to 1000:1 (Default: 100:1)</p>
        </div>
        ...
        <div class="bg-gray-50 rounded-lg p-4 space-y-2 text-sm">
          ...
          <div class="grid grid-cols-2 gap-2">
            ...
            <!-- NEW: Show leverage in preview -->
            <div>
              <span class="text-gray-600">Leverage:</span>
              <span class="font-medium">{{ form.leverage || 100 }}:1</span>
            </div>
          </div>
        </div>
...
const form = ref({
  ...
  password: '',
  balance: 10000,
  leverage: 100, // NEW: Default leverage
  is_admin: false,
  is_fake: true
})
...
const isFormValid = computed(() => {
  return form.value.first_name &&
         ...
         form.value.username.length >= 3 &&
         form.value.password.length >= 6 &&
         form.value.leverage >= 1 && 
         form.value.leverage <= 1000 && // NEW: Validate leverage range
         !usernameError.value
})
...
const submitForm = async () => {
  ...
  try {
    const userData = {
      ...
      is_admin: form.value.is_admin,
      balance: form.value.balance || 10000,
      leverage: form.value.leverage || 100, // NEW: Include leverage
      is_fake: form.value.is_fake
    }
    ...
    toastStore.success(`User ${newUser.username} created successfully with ${newUser.leverage}:1 leverage!`)
  } catch (error) {
    console.error('User creation error:', error)
    toastStore.error(handleApiError(error))
  } finally {
